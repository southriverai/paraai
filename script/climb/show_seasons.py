...
def main():
    ...
    repo_simple_climb = RepositorySimpleClimb.initialize_sqlite(path_dir_database)

    simple_climbs = repo_simple_climb.get_all()
    if simple_climbs:
        earliest = min(simple_climbs, key=lambda c: c.start_timestamp_utc)
        latest = max(simple_climbs, key=lambda c: c.start_timestamp_utc)
        earliest_dt = datetime.fromtimestamp(earliest.start_timestamp_utc, tz=timezone.utc)
        latest_dt = datetime.fromtimestamp(latest.start_timestamp_utc, tz=timezone.utc)
        print(f"Simple climbs: earliest {earliest_dt.strftime('%Y-%m-%d')}, latest {latest_dt.strftime('%Y-%m-%d')}")
    else:
        print("Simple climbs: (none)")
